...
const App = {
    ...
    async seedWeeklyTemplate() {
        console.log("[Apex] Seeding Weekly Template...");
        const days = 7;
        ...
        for (let i = 0; i < days; i++) {
            ...
            const template = [
                ...
                {time:'08:30', label:'Mission Focus Block 1', type:'academic'},
                {time:'14:00', label:'Mission Focus Block 2', type:'academic'},
            ...
            // Seed a default workout detail for the day
            await ApexDB.saveWorkoutPlan(date.getDay(), {
                name: "Elite Strength",
                focus: "Full Body",
                exercises: [
                    {n: "Squats", s: 4, r: "10"},
                    {n: "Pushups", s: 3, r: "Failure"},
                    {n: "Plank", s: 3, r: "60s"}
                ]
            });
        }
    ...
    async fetchWeeklyAIProtocol(key, constraints) {
        const prompt = `Generate a 7-day high-performance schedule and workout plan for ${this.state.profile.name}.
        Goals: ${this.state.goals.map(g => g.title).join(", ")}.
        Constraints: ${this.state.profile.constraints}.
        User specific adjustments for this week: ${constraints}.
        Output ONLY a JSON object with:
        "schedule": Array of tasks for each day (date, time, label, type)
        "workouts": Array of workout details (dayIndex 0-6, name, focus, exercises: [{n,s,r}])
        Be elite, high-density, and consistent.`;
        ...
        try {
            ...
            for (const w of result.workouts) await ApexDB.saveWorkoutPlan(w.dayIndex, w);
        } catch (e) {
    ...
    async sendCoachMessage(text) {
        ...
        this.showLoading("Coach Thinking...");
        try {
            const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
                method: "POST",
    ...
    async render() {
        ...
        if (taskList) {
            taskList.innerHTML = this.state.tasks.map(t => `
                <div class="glass-card p-5 flex items-center justify-between border-l-4 ${t.done ? 'border-green-500 opacity-30' : 'border-[#BA7517]'} transition-all">
                    <div class="flex-1">
                        <div class="flex items-center gap-2 mb-1.5">
                            <span class="text-[10px] font-black font-mono text-gray-500 italic">${t.time}</span>
                    ...
                    <button onclick="App.toggleTask(${t.id})" class="w-8 h-8 rounded-2xl border-2 ${t.done ? 'bg-green-500 border-green-500' : 'border-white/10'} flex items-center justify-center transition-all shadow-lg">
                        ${t.done ? '✓' : ''}
                    </button>
                </div>
            `).join('');
        ...
        // Workout Plan
        const plan = await ApexDB.getWorkoutPlan(this.state.selectedDay);
        if (plan) {
            ...
            exList.innerHTML = plan.exercises.map(ex => `
                <div class="p-4 glass-inset flex justify-between items-center border-l-2 border-white/10">
                    <span class="text-[12px] font-bold">${ex.n}</span>
                    <span class="text-[10px] font-black text-[#BA7517] font-mono">${ex.s} SETS · ${ex.r}</span>
                </div>
            `).join('');
        }
        ...
        if (goalList) {
            const html = this.state.goals.map(g => `
                <div class="glass-card p-6 border-b-2 border-${g.category === 'career' ? 'blue' : (g.category === 'physical' ? 'emerald' : 'orange')}-500/20">
                    <div class="flex justify-between items-center mb-4">
                        <span class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">${g.title}</span>
                        <span class="text-[11px] text-[#BA7517] font-black italic">${g.current}%</span>
                    </div>
                    <div class="w-full bg-black/40 h-1.5 rounded-full overflow-hidden shadow-inner">
                        <div class="bg-${g.category === 'career' ? 'blue' : (g.category === 'physical' ? 'emerald' : 'orange')}-500 h-full transition-all duration-1000 shadow-[0_0_15px_rgba(0,0,0,0.5)]" style="width: ${g.current}%"></div>
                    </div>
                </div>
            `).join('');
                ...
                setupGoalList.innerHTML = this.state.goals.map(g => `
                    <div class="flex justify-between items-center p-4 glass-inset">
                        <div class="flex flex-col">
                            <span class="text-[8px] font-black uppercase text-gray-600">${g.category}</span>
        ...
        if (chatBox) {
            chatBox.innerHTML = this.state.messages.map(m => `
                <div class="flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} mb-5">
                    <div class="max-w-[85%] p-4 rounded-3xl text-[12.5px] leading-relaxed ${m.role === 'user' ? 'bg-[#221606] border border-[#3d2208] text-[#f0a830] rounded-br-none' : 'glass-card text-gray-200 rounded-bl-none border-l-4 border-[#BA7517]'}">
                        ${m.role === 'assistant' ? '<div class="text-[8px] font-black text-[#BA7517] mb-2 uppercase tracking-widest font-mono italic">APEX_COACH</div>' : ''}
                        ${m.content}
                    </div>
                </div>
            `).join('');

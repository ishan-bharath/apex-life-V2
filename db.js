...
if (typeof Dexie === 'undefined') {
    console.error("[Apex] Dexie.js not found. IndexedDB functionality will be disabled.");
}

const db = new Dexie("ApexLifeV2DB");

// Define Schema for V2 Public Elite
db.version(2).stores({
    tasks: "++id, date, label, type, time, detail, done, [date+type]",
    logs: "++id, date, type, value, source", // type: 'mood', 'sleep', 'recovery', 'workout'
    ai_history: "++id, timestamp, role, content",
    goals: "++id, title, target, current, type, status", 
    metrics: "++id, date, height, weight, hr_rest, body_fat",
    settings: "id, value",
    profile: "id, name, age, height, weight, constraints"
});
...
const ApexDB = {
    ...
    async addLog(log) {
        return await db.logs.add({ date: new Date().toISOString().split('T')[0], ...log });
    },
